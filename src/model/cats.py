...
from .cats_mod import FeatureL2Norm, unnormalise_and_convert_mapping_to_flow
...
class CATs(nn.Module):
    ...
    freeze=True):
        ...
        # channels = [64] + [256] * 3 + [512] * 4 + [1024] * 23 + [2048] * 3 # resnet101
        channels = [64] + [256] * 3 + [512] * 4 + [1024] * 6 + [2048] * 3 #resnet50

        self.hyperpixel = hyperpixel
        if self.hyperpixel: # use hyperpixel as in https://github.com/juhongm999/hpf
            ...
            self.num_hyperpixel = len(hyperpixel_ids)
        else:
            self.proj = nn.ModuleList([
                nn.Linear(512, self.feature_proj_dim),
                nn.Linear(1024, self.feature_proj_dim),
                nn.Linear(2048, self.feature_proj_dim)
            ])
            self.num_hyperpixel = 1
        ...
        self.decoder = TransformerAggregator(
            ...
            num_hyperpixel=self.num_hyperpixel)
    ...
    def forward(self, tgt_feats, s_label, src_feats, v, ig_mask=None): 
        # Support -> target, query -> source;
        # v: [1, 512, 3600]
        # B, _, H, W = tgt_feats[0].size()
        B = 1
        H = 60
        W = 60
        ...
        for idx, feat in enumerate(tgt_feats):
            tgt_feats[idx] = F.interpolate(feat, self.feature_size, None, 'bilinear', True) ##!! brutal?
        for idx, feat in enumerate(src_feats):
            src_feats[idx] = F.interpolate(feat, self.feature_size, None, 'bilinear', True) ##!! brutal?

        corrs = []
        src_feats_proj = []
        tgt_feats_proj = []
        for i, (src, tgt) in enumerate(zip(src_feats, tgt_feats)):
            # Skip layer 2, 3 only 4:
            if i == 2:

                # mask target (support)img
                s_mask = F.interpolate(s_label.unsqueeze(1).float(), size=tgt.shape[-2:], mode='nearest')
                s_mask = (s_mask > 1).float()
                tgt = tgt * s_mask

                corr = self.corr(self.l2norm(src), self.l2norm(tgt))
                corrs.append(corr)
                src_feats_proj.append(self.proj[i](src.flatten(2).transpose(-1, -2))) # [1,3600,512] -> [1,3600,128 (feature_proj_dim)]
        ...
        refined_corr = F.interpolate(refined_corr.unsqueeze(1), size=[H*W, H*W], mode='bilinear', align_corners=True) ##!! brutal

        refined_corr = refined_corr.squeeze(1)

        if ig_mask is not None:
            ig_mask = ig_mask.view(B, -1, H*W).expand(refined_corr.shape)
            refined_corr[ig_mask==True] = 0.0001         # [B, N_q, N_s]
        ...
        # return weighted_v, refined_corr
        return weighted_v
